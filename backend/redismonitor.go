...
func (b *redisMonitor) healthcheckData(host string) (healthcheckData, error) {
...
func (b *redisMonitor) check(host, backend string) bool {
	healthcheckData, err := b.healthcheckData(host)
	if err != nil {
		return false
	}
	url := fmt.Sprintf("%s/%s", backend, strings.TrimLeft(healthcheckData.path, "/"))
	...
	if healthcheckData.status != 0 && rsp.StatusCode != healthcheckData.status {
		return false
	}
	if healthcheckData.body != "" {
		data, _ := ioutil.ReadAll(rsp.Body)
		return strings.Contains(string(data), healthcheckData.body)
	}
	return true
}
