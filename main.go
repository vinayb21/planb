...
import (
	...
	"github.com/vinayb21/planb/backend"
	"github.com/vinayb21/planb/reverseproxy"
	"github.com/vinayb21/planb/router"
	"github.com/vinayb21/planb/tls"
)
...
func startProfiling() {
	...
	_ = pprof.Lookup("block").WriteTo(lockfile, 0)
...
func main() {
	...
	app.Flags = []cli.Flag{
		...
		cli.StringFlag{
			Name: "tls-preset",
			Usage: fmt.Sprintf(
				`Preset containing supported TLS versions and cyphers, according to https://wiki.mozilla.org/Security/Server_Side_TLS.
Possible values are [%s, %s, %s]`,
				router.TLS_PRESET_MODERN,
				router.TLS_PRESET_INTERMEDIATE,
				router.TLS_PRESET_OLD,
			),
...
The value 'none' can be used to disable access logs.`,
		...
		cli.StringFlag{
			Name:  "engine",
			Value: "native",
			Usage: "Reverse proxy engine, option is 'native'",
		},
		cli.BoolFlag{
			Name:  "backend-cache",
	...
	app.Author = "HackerRank team"
	app.Email = "vinay@hackerrank.com"
	_ = app.Run(os.Args)
}
